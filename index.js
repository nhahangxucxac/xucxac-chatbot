...
// Phiên bản: 4.0 — Fix lọc tin nhắn cũ + /takeover
// ============================================================
// Cách chỉnh nội dung:
//   1. Thông tin quán       → tìm "THÔNG TIN NHÀ HÀNG"
//   2. Menu & giá           → tìm "MENU"
//   3. Chương trình KM      → tìm "CÁC CHƯƠNG TRÌNH"
//   4. Câu hỏi thường gặp  → tìm "FAQ"
// ============================================================
...
const TAKEOVER_DURATION   = 30 * 60 * 1000; // 30 phút
const MSG_EXPIRE_MS       = 30 * 1000;       // Bỏ qua tin cũ hơn 30 giây
...
app.post('/webhook', async (req, res) => {
  ...
  for (const entry of body.entry) {
    for (const event of entry.messaging) {
      ...
      // ── Lọc tin nhắn cũ hơn 30 giây ──────────────────────
      const msgTime = event.timestamp;
      if (Date.now() - msgTime > MSG_EXPIRE_MS) {
        console.log(`⏭ Bỏ qua tin cũ (${Math.round((Date.now() - msgTime)/1000)}s): ${text}`);
        continue;
      }

      console.log(`📩 Tin nhắn từ ${senderId}: ${text}`);

      // ── Lệnh nhân viên: /takeover ──────────────────────────
      ...
      if (takeoverSessions[senderId]) {
        ...
        if (elapsed < TAKEOVER_DURATION) {
          ...
          delete takeoverSessions[senderId];
          console.log(`⏰ Takeover hết hạn, bot tiếp quản: ${senderId}`);
        }
      }

      // ── Xử lý đặt bàn hoặc AI ─────────────────────────────
...
// ============================================================
// XỬ LÝ AI RESPONSE — GEMINI 2.0 FLASH
// ============================================================
...
app.listen(PORT, () => {
  console.log(`🎲 Xúc Xắc Chatbot v4.0 (Gemini) đang chạy tại cổng ${PORT}`);
});
